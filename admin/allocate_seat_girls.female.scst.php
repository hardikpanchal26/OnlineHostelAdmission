...
<?php 
    ...
    $factor = $data['factor']/100;
      ...
      $girls_limit= (int) ($percent['scst']/100 * $data['total_girls']);
      
      $result=mysqli_query($connection,"SELECT * FROM `determining_factors` WHERE gender='female' AND allot>0 AND college='$college' AND (category='sc' OR category='st' )");
      $allotedseats=mysqli_num_rows($result);

      $query="SELECT AVG(`distance`) as `avgdist`, AVG(`merit`) as `avgmerit` FROM `determining_factors` WHERE gender='female' AND allot=0 AND (category='sc' OR category='st'  )";
      ...
      $avg= mysqli_fetch_array($result,MYSQLI_ASSOC);
       ...
       $query = mysqli_query($connection,"SELECT *, ('$factor')*(`distance`-'$Dmean')+ (1-'$factor')*('$Mmean'-`merit`) as `finalmerit` FROM `determining_factors` WHERE gender='female' AND allot=0 AND college='$college' AND (category='sc' OR category='st' ) ORDER BY `finalmerit` DESC LIMIT $limit;"); 
    ...
    header("location: allocate_seat_girls.female.others.php");
