...
<?php 
    ...
    $factor = $data['factor']/100;
      ...
      $boys_limit= (int) ($percent['sebc']/100 * $data['total_boys']);
      
      $result=mysqli_query($connection,"SELECT * FROM `determining_factors` WHERE gender='male' AND allot>0 AND college='$college' AND category='sebc'");
      $allotedseats=mysqli_num_rows($result);

      $query="SELECT AVG(`distance`) as `avgdist`, AVG(`merit`) as `avgmerit` FROM `determining_factors` WHERE gender='male' AND allot=0 AND college='$sebc'  AND category='general'";
      ...
      $avg= mysqli_fetch_array($result,MYSQLI_ASSOC);
       ...
       $query = mysqli_query($connection,"SELECT *, ('$factor')*(`distance`-'$Dmean')+ (1-'$factor')*('$Mmean'-`merit`) as `finalmerit` FROM `determining_factors` WHERE gender='male' AND allot=0 AND college='$college'  AND category='sebc' ORDER BY `finalmerit` DESC LIMIT $limit;"); 
    ...
    header("location: allocate_seat_boys.male.scst.php");
