...
<?php 
    ...
    $factor = $data['factor']/100;
      ...
      $avg= mysqli_fetch_array($result,MYSQLI_ASSOC);
       ...
       $query = mysqli_query($connection,"SELECT *, ('$factor')*(`distance`-'$Dmean')+ (1-'$factor')*('$Mmean'-`merit`) as `finalmerit` FROM `determining_factors` WHERE gender='male' AND allot=0 AND college='$college' ORDER BY `finalmerit` DESC LIMIT $limit;"); 
